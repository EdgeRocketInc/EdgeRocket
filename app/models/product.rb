...
class Product < ActiveRecord::Base
  ...
  def self.search_courses(filter)
    ...
    self.connection.select_all('select p.name as pname, p.authors, p.origin, p.price, v.name as vname, v.logo_file_name, p.keywords, p.school from products p left join vendors v on p.vendor_id=v.id order by p.name')
  end
end
